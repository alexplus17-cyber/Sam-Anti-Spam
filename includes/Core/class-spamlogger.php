...
class SpamLogger {
	...
	public function delete_logs( array $ids ) {
		...
		$format = implode( ',', array_fill( 0, count( $sanitized_ids ), '%d' ) );
		$sql    = $wpdb->prepare( "DELETE FROM $table_name WHERE id IN ($format)", ...$sanitized_ids );
	...
	public function get_logs_by_ids( array $ids ) {
		...
		$format = implode( ',', array_fill( 0, count( $sanitized_ids ), '%d' ) );
		$sql    = $wpdb->prepare( "SELECT * FROM $table_name WHERE id IN ($format)", ...$sanitized_ids );
